...
def index(request):
    ...
    user = request.user
    context = {
        "exercises": SetOfExercise.objects.compute_report_for_user(
            user=user,
        ...
        "interval_statistics": SetOfExercise.objects.compute_report_for_user(
            user=user,
            start_date=start_date,
            end_date=end_date,
            periodicity="total",
...
def chart(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            workouts = SetOfExercise.objects.compute_report_for_user(
                user=user,
                start_date=start_date,
                end_date=end_date,
                periodicity="daily",
